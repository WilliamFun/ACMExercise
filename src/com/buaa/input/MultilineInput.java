...
public class MultilineInput {
    ...
    public static void main(String[] args) {
        ...
        while(sc.hasNext()){
            ...
            long sum=0;
            for(int[] a:nums) {//每门课的平时成绩加起来
                sum+=a[0];
            }
            long limit=avg*n;//至少要达到的总分数
            int index=0;
            long time=0;
            while(sum<limit){
                int tmp=full-nums[index][0];//当前科目距离满分差距
                if(tmp+sum<=limit){                  //如果一门课程复习到满分，小于限制，就把它学到满分
                    time+=tmp*nums[index][1];
                    sum+=tmp;
                    index++;
                }
                else{                              //如果一门课程复习到满分，大于限制，就学到限制
